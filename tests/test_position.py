...
def test_close_position_full_long():
    ...
    assert result["profit"] == pytest.approx(89.0)  # (110 - 100) * 10
...
def test_close_position_partial_long():
    ...
    assert result["profit"] == pytest.approx(44.5)  # Half the quantity
...
def test_close_position_full_short():
    ...
    assert result["profit"] == pytest.approx(91)  # (100 - 90) * 10 * (1 - 0.01)
    assert result["remaining"] == 0
...
def test_close_position_partial_short():
    ...
    assert result["profit"] == pytest.approx(45.5)
